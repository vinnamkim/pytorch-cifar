    ...
    for layer in model.modules():
        ...
        if layer_type == 'Linear':
            ...
            A = torch.nn.functional.unfold(
                ...
                stride=layer.stride if layer.stride is not None else 1,
                padding=layer.padding if layer.padding is not None else 0,
            )
...
def _make_square(A):
    if A.shape[1] < A.shape[2]:
        return A.bmm(A.transpose(1, 2))
    else:
        return A.transpose(1, 2).bmm(A)

def compute_cond1(model: nn.Module, loss_type: str = 'mean') -> None:
    """
    Compute per-example gradients and save them under 'param.grad1'. Must be called after loss.backprop()
    ...
    for layer in model.modules():
        ...
        if layer_type == 'Linear':
            ...
            s1, _ = torch.symeig(_make_square(A))
            s2, _ = torch.symeig(_make_square(B))

            s1 = 0.5 * s1.log().cpu().numpy()
            s2 = 0.5 * s2.log().cpu().numpy()

            forwards_cond = []
            for s in s1:
                s = s[np.isfinite(s)]
                forwards_cond.append((s[0], s[-1]))

            backwards_cond = []
            for s in s2:
                s = s[np.isfinite(s)]
                backwards_cond.append((s[0], s[-1]))

            setattr(layer, 'forwards_cond', forwards_cond)
            setattr(layer, 'backwards_cond', backwards_cond)
            # grad1 = torch.einsum('ijk,ilk->ijl', B, A)
            # shape = [n] + list(layer.weight.shape)
            # setattr(layer.weight, 'grad1', grad1.reshape(shape))
            # if layer.bias is not None:
